...
// @ts-ignore
interface ReviewQueryMiddleware extends mongoose.Query<any> {
  r?: ReviewDoc;
}
...
// findByIdAndUpdate
// findByIdAndDelete
reviewSchema.pre<ReviewDoc>(/^findOneAnd/, async function(next) {
  // @ts-ignore
  this.r = await this.findOne();
  next();
});

reviewSchema.post<ReviewDoc>(/^findOneAnd/, async function() {
  // @ts-ignore
  await this.r.constructor.calcAverageRatings(this.r.tour);
});
